...
namespace OrakYazilimLib.DbUtil
{
  ...
  public class FiMssqlu
  {
    ...
    private Fdr SqlExecuteNonQuery(FiQuery fiQuery)
    {
      using SqlConnection sqConn = new SqlConnection(connString);
      string query = FiQueryUtils.FixSqlProblems(fiQuery.sql);
...
//command.Dispose();
    ...
    public Fdr<T> SqlExecuteScalar<T>(string sql, List<FiSqlParameter> listParam)
    {
      ...
      if (typeof(T) == typeof(int))
      {
        ...
        if (result == null)
        {
          fdrMain.obReturn = (T)(object)returnValueForNull;
          return fdrMain;
        }
      }
      ...
      { // convert : sqlden dönen değer T tipinde değilse dönüşüm yapar ??? hata çıkabilir
        ...
        fdrMain.obReturn = (T)Convert.ChangeType(result, typeof(T));
      }
...
// public Fdr<DataTable> SqlExecuteSelectOld(FiQuery fiQuery)
// {
//   DataSet ds = new DataSet();
